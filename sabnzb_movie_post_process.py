...
class SABResult(object):
    ...
    def __repr__(self):
        return '<SABResult result: {0} data: {1} error: {2} convert: {3} video_track: {4} ' \
            'audio_track: {5}>'.format(self.result, self.data, self.error, self.convert,
...
def backup_original_file(folder, file):
    ...
    backup_name = '{0}{1}'.format(file.name, '.original')
...
def convert_mkv_file(folder, file, video_track, audio_track):
    ...
    if convert_success.result:
        convert_success = create_output_file(
            convert_success.data, file, video_track, audio_track)
...
def main():
    ...
    if validate_mkv_result.result:
        if validate_mkv_result.convert:
            convert_result = convert_mkv_file(sab_directory, validate_mkv_result.data,
                                              validate_mkv_result.video_track, validate_mkv_result.audio_track)
